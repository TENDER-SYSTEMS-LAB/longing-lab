...
- **How the `Joint` line is computed and whether it is displayed.** The order itself is decided — see [[DEC-010-declared-order-with-a-joint-line]] — but the overlap line it adds is specified for one swap only, and its display status, like `Unexplained`'s, is open. See [[attribution-ledger]].
