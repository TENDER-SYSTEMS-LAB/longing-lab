...
- **Whether the `Joint` line is displayed, and what is permuted.** Its computation is decided — the range over every admissible order, [[DEC-010-declared-order-with-a-joint-line]] — but its display is unset, and the range needs a statement of whether multi-line blocks (waves, priced factors) are fitted jointly so that only blocks are ordered; the assistant's proposals (blocks fitted jointly, `Joint` shown as a width beside each line) were put on 2026-09-20 and not answered. Whether analysts read through orders of their own is the user's proposal, `user-originated`, not adopted; the constraint any such reading would obey is decided — [[DEC-012-analyst-orders-inside-the-house-constraint]] — and which order each would carry, how a monthly reading is cut from weekly ledgers, and whether it is shown line by line are not. Also open: where the numeraire revision appears on the audience receipt, and the receipt's published wording. See [[DEC-011-the-audience-ledger-reads-hearts-cooling]].
