...
- **The AI wave's reset connection coefficient, and the rule that sorts an act between execution and judgment.** Restoration now counts as arrival and substitution counts judgment, [[DEC-014-restoration-is-arrival-and-substitution-is-judgment]], so the AI coefficient set from substitution alone is wrong by construction and unset. The user names an ambiguous middle — execution with a restoring side, execution that hands over the deliberation whole — and proposes a per-security ontology of execution and judgment, `user-originated`, not written. The measurement design on [[data-sources]] is `llm-proposed` and not redrawn against the decision. See [[technology-waves]].
