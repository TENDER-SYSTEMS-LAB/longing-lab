...
- **How the `Joint` line is computed and whether it is displayed.** The order itself is decided — see [[DEC-010-declared-order-with-a-joint-line]] — but the overlap line it adds is specified for one swap only and its display is unset; the assistant's proposals (range over the six admissible orders, blocks fitted jointly, shown as a width beside each line) were put on 2026-09-20 and not answered. Also open: where the numeraire revision appears on the audience receipt, and the receipt's published wording. See [[DEC-011-the-audience-ledger-reads-hearts-cooling]].
