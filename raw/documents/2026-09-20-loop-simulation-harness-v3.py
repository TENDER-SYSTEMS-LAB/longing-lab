...
THE WAVE DATES ARE PLACEHOLDERS. They are shaped after the real order of arrival
in the three countries, but no statistical series has been collected and no
normalisation rule exists. Nothing here is calibrated against anything.
"""
...
#        name          KR    JP    US   lag  strength creates
WAVES = [("dialup",     10,  -40,  -80, 260, 0.9,   1.00),
         ("mobile",    150,   50,  240, 300, 1.2,   1.30),
         ("broadband", 205,  350,  380, 280, 1.0,   0.85),
         ("socialweb", 195,  450,  430, 300, 1.1,   1.40),
         ("smartphone",690,  700,  675, 240, 1.6,   0.55),
         ("messaging", 745,  760,  750, 260, 1.3,   0.90),
         ("ai",       1355, 1358, 1352, 130, 2.2,   0.12)]

MEASURED_UNTIL = 1540   # the institute publishes this boundary; beyond it the
                        # same functional form is extrapolated
...
def wave_state(t, m_i, p):
    ...
    """
    arrive = subst = 0.0
    for name, kr, jp, us, lag, strength, creates in WAVES:
        t0 = (kr, jp, us)[m_i]
        a = logistic(t - t0, p.spread)
        arrive += strength * creates * (a - logistic(t - 1 - t0, p.spread))
        subst += strength * logistic(t - t0 - lag, p.spread * 1.6)
...
def tests():
    ...
    print("\n" + "=" * 76)
    print("LONGING RESEARCH — DEC-008 / DEC-009 / technology-waves")
    print("=" * 76)
